...
module.exports = {
    ...
    options: [
        ...
        },
        {
            type: "STRING",
            name: "wiadomosc",
            description: "Jaki opis ma mieć embed powitań. Zmienne {member}, {guild}",
            required: false,
        }
    ],
    ...
    async run(interaction, blad, sukces, db) {
        try{
            ...
            if(mode === `reset`){
                if(!istnienie) return sukces(`System Powitań nie był właczony`, interaction)
                await db.destroy({ where: { guildId: interaction.guild.id } })
                return sukces(`System Powitań został wyłączony!`, interaction)
            }
            ...
            if(mode === 'set' && wiadomosc && !kanal){
                ...
                if(!istnienie){
                    ...
                    return sukces(`Opis powitań ustawiono na \`\`${wiadomosc}>\`\`!`, interaction)
                }
                else{
                    ...
                    return sukces(`Opis powitań zmieniono na \`\`${wiadomosc}>\`\`!`, interaction)
                }
            }
